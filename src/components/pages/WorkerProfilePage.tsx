...
export function WorkerProfilePage() {
  ...
  const [stats, setStats] = useState({ totalAssignments: 0, thisMonthDays: 0, teamsCount: 0 });
  ...
  const [attendanceHistoryMap, setAttendanceHistoryMap] = useState<Map<string, AttendanceHistory>>(new Map());
  ...
  const fetchWorkerProfile = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Today Assignments:', data.todayAssignments);
        setProfile(data.profile);
        setTodayAssignments(data.todayAssignments || []);
        setStats(data.stats || { totalAssignments: 0, thisMonthDays: 0, teamsCount: 0 });
        setTeams(data.teams || []);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.attendance && data.attendance.length > 0) {
          ...
          // Get the most recent attendance for each assignment (active check-in)
          const assignmentGroups = new Map<string, Attendance[]>();

          data.attendance.forEach((att: Attendance) => {
            if (!assignmentGroups.has(att.assignmentId)) {
              assignmentGroups.set(att.assignmentId, []);
            }
            assignmentGroups.get(att.assignmentId)!.push(att);
          });

          // For each assignment, get the most recent record
          assignmentGroups.forEach((records, assignmentId) => {
            const sorted = records.sort((a, b) =>
              new Date(b.checkInTime).getTime() - new Date(a.checkInTime).getTime()
            );
            map.set(assignmentId, sorted[0]);
          });
  ...
  const fetchAttendanceHistory = async (assignmentId: string) => {
    try {
      const today = format(new Date(), 'yyyy-MM-dd');
      const response = await fetch(`/api/attendance/history?assignmentId=${assignmentId}&date=${today}`);
      if (response.ok) {
        const data = await response.json();
        setAttendanceHistoryMap(prev => new Map(prev).set(assignmentId, data));
      }
    } catch (error) {
  ...
  const handleCheckIn = async (assignmentId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setAttendanceMap(prev => new Map(prev).set(assignmentId, data.attendance));
        // Refresh history
        fetchAttendanceHistory(assignmentId);
      } else {
  ...
  const handleCheckOut = async (assignmentId: string) => {
    const attendance = attendanceMap.get(assignmentId);
    if (!attendance) {
  ...
  const toggleHistory = (assignmentId: string) => {
    if (showHistory === assignmentId) {
      ...
      if (!attendanceHistoryMap.has(assignmentId)) {
        fetchAttendanceHistory(assignmentId);
      }
    }
  };
  ...
  const setSiteWorkersDialogDate = (dateStr: string) => {
    setSiteWorkersDialog((prev) =>
      prev ? { ...prev, date: dateStr } : null
    );
  };
    ...
    const fetchSiteWorkers = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setSiteWorkersList(data.workers || []);
        } else {
          setSiteWorkersList([]);
        }
      } catch {
  ...
  // Get first attendance for header stats (if any)
  const firstAttendance = attendanceMap.size > 0 ? Array.from(attendanceMap.values())[0] : null;
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex gap-2 p-1 bg-slate-100 rounded-xl">
          <button
            ...
            className={`flex-1 py-3 px-4 rounded-lg font-medium transition-all ${activeTab === 'assignments'
          ...
          <button
            ...
            className={`flex-1 py-3 px-4 rounded-lg font-medium transition-all ${activeTab === 'teams'
              ? 'bg-white text-slate-900 shadow-sm'
              : 'text-slate-600 hover:text-slate-900'
              }`}
          >
          ...
          <>
            ...
            {todayAssignments.length > 0 ? (
              todayAssignments.map((assignment, index) => {
                const attendance = attendanceMap.get(assignment.id);
                ...
                return (
                  <Card key={assignment.id} className="p-6 shadow-sm border-0 bg-white">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          {assignment.site.city && (
                            <p className="text-sm text-slate-500">{assignment.site.city}</p>
                          )}
                          {assignment.notes && (
                    ...
                    {/* View all workers in site – only when history is enabled for this worker */}
                    {assignment.showAssignmentHistory && (
                      <Button
                        onClick={() => openSiteWorkersDialog(assignment.site.id, assignment.site.name)}
                        variant="outline"
                        className="w-full rounded-xl h-10 border-slate-200 text-brand-600 hover:bg-brand-50"
                      >
                    ...
                    {/* Attendance History */}
                    {showHistory === assignment.id && attendanceHistoryMap.has(assignment.id) && (
                      <div className="mt-4 p-4 bg-slate-50 rounded-xl">
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="font-semibold text-slate-900">Today's History</h4>
                          <Badge variant="outline" className="text-xs">
                            {attendanceHistoryMap.get(assignment.id)!.summary.totalTimeFormatted}
                          </Badge>
                        </div>

                        <div className="space-y-2">
                          {attendanceHistoryMap.get(assignment.id)!.history.map((record, index) => {
                            const duration = record.checkOutTime
                              ? Math.floor(
                                (new Date(record.checkOutTime).getTime() -
                                  new Date(record.checkInTime).getTime()) / (1000 * 60)
                              )
                              : null;
                                ...
                                <div className="flex items-center gap-3">
                                  <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                                    <span className="text-blue-600 font-semibold text-xs">
                                      #{attendanceHistoryMap.get(assignment.id)!.history.length - index}
                                    </span>
                                  </div>
                                  <div>
                        ...
                        {attendanceHistoryMap.get(assignment.id)!.history.length === 0 && (
                          <p className="text-center text-slate-500 text-sm py-4">
                            No check-in history for today
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border border-blue-200">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {team.teamLead.phone && (
                            <p className="text-xs text-slate-500 mt-1">{team.teamLead.phone}</p>
                          )}
                        </div>
                      </div>
        ...
        {/* Team Members - OLD SECTION TO REMOVE */}
        {false && teams.length > 0 && teams.map((team) => (
          <Card key={team.id} className="p-6 shadow-sm border-0 bg-white">
          </Card>
        ))}
            ...
            {siteWorkersDialog && (
              <>
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  <Button
                    size="sm"
                    variant={siteWorkersDialog.date === 'all' ? 'default' : 'outline'}
                    className={siteWorkersDialog.date === 'all' ? 'bg-brand-600 hover:bg-brand-700' : ''}
                    onClick={() => setSiteWorkersDialogDate('all')}
                  >
                    All
                  </Button>
                  {(['Today', 'Yesterday', '2 days ago', '3 days ago'] as const).map((label, i) => {
                    ...
                    return (
                      <Button
                        ...
                        className={isActive ? 'bg-brand-600 hover:bg-brand-700' : ''}
                        onClick={() => setSiteWorkersDialogDate(dateStr)}
                      >
                        {label}
                      </Button>
                    );
                ...
                <p className="text-sm text-slate-500 mt-1">
                  {siteWorkersDialog.date === 'all'
                    ? 'All workers assigned to this site'
                    : format(new Date(siteWorkersDialog.date), 'EEEE, MMM d, yyyy')}
                </p>
              </>
            )}
            {siteWorkersLoading ? (
              ...
              <p className="text-center py-6 text-slate-500 text-sm">
                {siteWorkersDialog?.date === 'all'
                  ...
                  : 'No workers found for this site on the selected date.'}
              </p>
            )}
          </DialogContent>
        </Dialog>
